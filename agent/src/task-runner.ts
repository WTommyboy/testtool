...
const buildPrompt = (
  ...
  return [
    ...
    "- Save/reopen + CSV flow policy: if a required earlier subcondition such as state restoration or current preview fails, judge that failure directly and write CSV comparison as not reached. A missing CSV after an already-broken workflow must not hide a functional regression as BLOCKED. If helper plan still has save/reopen/download actions available, request Tool Bridge/continuation before judging the CSV path as not reached.",
    "- CSV verification policy: after a visible UI download succeeds, read the local downloaded CSV directly; do not use Google Sheets as the formal evidence path.",
    "- Metadata compare policy: use `input/reference-index.json` to find `bi_metadata_csv`, normally `rules/BI_DATA/metadata.csv`, and confirm the testcase source filename when specified; do not search all uploaded CSV files or call BI APIs to fill the expected list.",
