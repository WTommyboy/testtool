...
const writeRunBrief = (
  ...
  const content = [
    ...
    "- For frontend observation cases, if helper evidence includes `frontend-observation-evidence.json`, judge from its structured controls/dialogs/toastLike/datePicker/operations evidence. Do not require preview, selected metric fields, chart, table, or network evidence unless the testcase explicitly asks for backend/preview data.",
    "- `collage.observeFrontendState` evidence can support PASS or FAIL for `測試標的=前端呈現/功能流程`: UI state matches expectation -> PASS; under-test UI state/action is visibly wrong -> FAIL. Use BLOCKED only for page/target/evidence unavailability, not for observed product UI failures.",
