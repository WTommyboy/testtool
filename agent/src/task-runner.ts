...
const writeRunBrief = (
  ...
  const content = [
    ...
    "9. Read `input/rule-index.json` and load the current-case mandatory/recommended rule shortlist; do not read every rule file unless a blocker requires exact policy text.",
    biMetadataCsv
    ...
    "- For save/reopen + CSV cases, judge required subconditions in order. If save/reopen state restoration or current preview already failed, record that primary failure and mark CSV comparison as not reached; do not downgrade the known functional failure to BLOCKED merely because the later CSV download could not run.",
    "- For metadata/dropdown cases, use the canonical `rules/BI_DATA/metadata.csv` path from `input/reference-index.json` when present, and record reference_csv/source_report/match_key/compare_fields in detail_json.",
...
const buildPrompt = (
  ...
  return [
    ...
    "- Read `input/current-case-pack.json.mustReadRuleKeys`, then `input/rule-index.json`, and load the mandatory/recommended rule files for the current case. This is intentionally slower than relying only on compact summaries.",
    "- Treat `rules/PROJECT_AGENTS_FULL.md` and `rules/BI_TEST_RULES/` as binding BI domain references for case interpretation. For metadata comparison, the canonical copied CSV is `rules/BI_DATA/metadata.csv` when present.",
    ...
    "- Save/reopen + CSV flow policy: if a required earlier subcondition such as state restoration or current preview fails, judge that failure directly and write CSV comparison as not reached. A missing CSV after an already-broken workflow must not hide a functional regression as BLOCKED.",
    "- Metadata compare policy: use `input/reference-index.json` to find `bi_metadata_csv`, normally `rules/BI_DATA/metadata.csv`; do not search for alternate metadata files or call BI APIs to fill the expected list.",
    ...
    "- CSV/download detail_json should include downloadedCsv path/filename and previewComparison when reached; if not reached because an earlier workflow subcondition failed, include csv_comparison_status=\"not_reached\" and the failed subcondition.",
    "- Metadata/detail_json should include reference_csv, source_report, match_key, compare_fields, normalizationNotes, missingFields, and extraFields when the case compares UI fields to metadata.",
