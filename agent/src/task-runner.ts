...
const writeRunBrief = (
  ...
  const content = [
    ...
    "- A test-package-consistency error is a whole-package design signal. Do not let future-case package issues block the current case unless document-consistency keeps them as current-case errors.",
    "- For auto-advanced cases where currentCaseSelection.source is `agent_case_progress` or `tool_response_state`, do not create Tool Bridge ambiguity solely because earlier rows in input/testcase.xlsx are empty; use output/agent-case-progress.json as dispatch history and judge only the selected current case.",
...
const buildPrompt = (
  ...
  return [
    ...
    "- If `input/test-package-consistency.json` has status=error but `input/document-consistency.json` is ok/warning, continue with the selected current case and record the package issue only as context.",
    "- If `input/document-consistency.json.startCaseDocumentGate.enabled` is false, precedingCases are informational only; do not ask PM whether to skip earlier workbook rows.",
