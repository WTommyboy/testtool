...
const buildPrompt = (
  ...
  return [
    ...
    "- CSV verification policy: after a visible UI download succeeds, read the local downloaded CSV directly; do not use Google Sheets as the formal evidence path. If the testcase specifies project/report-list download, do not reopen the editor; compare the downloaded CSV with the pre-save preview evidence.",
    ...
    "- CSV/download detail_json should include downloadedCsv path/filename, download trigger/scope and previewComparison when reached; if not reached because an earlier workflow subcondition failed, include csv_comparison_status=\"not_reached\" and the failed subcondition.",
