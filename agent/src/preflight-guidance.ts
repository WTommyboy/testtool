...
export const writePreflightGuidance = (runDir: string, devUrl: string | null): string => {
  ...
  const content = [
    ...
    "Preflight 只允許做以下事情:",
    "",
    "- 先呼叫 Playwright MCP `browser_tabs` list；成功回傳即代表 browser tooling available。",
    "- 開啟 DEV URL。",
    ...
    "",
    "不可在尚未嘗試 `browser_tabs` 前自行判定 `TOOL_EXECUTION_UNAVAILABLE`。若 `browser_tabs` 成功,後續 blocker 應依實際 UI/SSO/證據不足分類,不可再寫 browser tool unavailable。",
    "",
    "```text",
