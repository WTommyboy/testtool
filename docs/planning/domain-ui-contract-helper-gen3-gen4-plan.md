...
Status: planned, dev-tracked; Gen1a/Gen1b contract wiring and short-term bridge are in dev; Gen1c contract hardening added before next live UAT; scope-aware contract refinement recorded after run `81455108-f612-4024-9d71-4f1995e08d7a`; P0 scope runtime guards and observation bridge are in dev; P0 vocabulary-first reset recorded after run `1762c1b2-bc42-47f0-80b6-d1ef10a0f713`; platform/domain/testcase boundary rule formalized 2026-05-19; P0.27 report mutation flow compatibility slice implemented 2026-05-20; P0.28 reduced blocked-smoke stabilization slice implemented 2026-05-20 after run `12594ce2-1555-432d-800c-3663934913a1`; P0.29 BI official inline formula-builder domain/testcase alignment recorded 2026-05-20; P0.31 result evidence gate non-terminal containment fix recorded after run `5dbc8fa5-1843-4980-9729-3bd7ac4136b7`; P0.32 Agent control WebSocket active-run resilience regression fix recorded after run `ff1fccf2-9350-4187-8837-42c808fe631d`; P0.33 reduced-smoke live blocker bridge recorded after run `10e944dc-4d9b-4e32-b197-9a8339dd52e5`; P0.34 Codex no-result containment recorded after run `071a690a-314e-4d93-8589-84772c359d6e`; P0.35 server-side agent disconnect non-terminal policy recorded after run `0c1ce924-a4bd-410a-be99-0f880cd525be`; P0.37 reduced-smoke blocked judgment/order bridge recorded after run `3fe0f31b-a0ca-47f7-9712-925531fcc8c0`; P0.38 final reduced-smoke blocked bridge recorded after run `f5196a8d-edb0-42e5-830f-3ef9efc99bec`
Primary triggers: BIUI_COLLAGE_R001 runs `67990303-2c1b-4559-924a-297a089b5949`, `81455108-f612-4024-9d71-4f1995e08d7a`, `ecc53283-18c4-4b29-a405-e7681626e28b`, `1762c1b2-bc42-47f0-80b6-d1ef10a0f713`, `12594ce2-1555-432d-800c-3663934913a1`, and `10e944dc-4d9b-4e32-b197-9a8339dd52e5`
...
- P0.22: isolate result upload transport failures. Status 2026-05-19: implemented after run `01e9c8e6-07b5-45ba-9309-48e3863091dd` stopped at B-04 even though Codex had written trusted `output/result.xlsx`. Result workbook upload now retries transient transport/server failures, and partial diagnostics preserve an existing trusted result workbook instead of overwriting the local status as agent-fallback-only. Verification entrypoint: `npm run verify:result-upload-retry`; the smoke now covers both socket/network `fetch failed` and HTTP 503 before success.
- P0.21 follow-up: isolate single-case result evidence upload rejections. Status 2026-05-19: implemented after 108-case run `ee0cae6b-f2c4-4cb0-a5aa-d79f5b611ca3` stopped at N-02 with a local single-case workbook but server `RESULT_EVIDENCE_GATE_FAILED` issue codes `RESULT_FRONTEND_OBSERVATION_VISUAL_FALLBACK_REQUIRED` and `TOOL_BRIDGE_RESPONSE_MISSING`. Agent now catches containable result evidence gate upload failures for `codex_generated` single-case workbooks, rewrites only the current row, writes `output/result-evidence-upload-containment.json`, and retries upload. Visual fallback disputes are classified as `BLOCKED_NEEDS_VISUAL_REVIEW`; other containable gate disputes use `BLOCKED_RESULT_GATE_CONTAINMENT`. Hard structural result-gate errors still abort. Verification entrypoint: `npm run verify:result-evidence-upload-containment`, plus actual N-02 copy smoke and existing `npm run verify:result-evidence-gate`.
- P0.23: classify 108-case BLOCKED results before the next runtime batch. Status 2026-05-19: implemented as `npm run verify:p0-108-blocked-triage` against run `ee0cae6b-f2c4-4cb0-a5aa-d79f5b611ca3`. Baseline is 108 total / 48 BLOCKED, split into result-gate visual fallback contract gaps, save/reopen/row-download flow gaps, missing action-template/helper-flow gaps, manual-review items, domain metadata/field-picker gaps, wrong observation route/missing case contracts, date helper/product gaps, navigation gap, and one testcase precondition gap. This is a triage smoke only; runtime fixes must still be implemented category by category with their own small smoke.
- P0.24: tighten frontend visual fallback contract before changing another live route. Status 2026-05-19: implemented as `npm run verify:p0-visual-fallback-contract` plus result-gate/upload-containment regression. The fixture covers the P0.23 `result_gate_visual_fallback_contract_gap` bucket by sampling `J-03`, `K-02`, and `N-01`: ordinary screenshot-only `EVIDENCE_INSUFFICIENT` is rejected, incomplete visual fallback markers are rejected, and complete `BLOCKED_NEEDS_VISUAL_REVIEW` detail_json passes only when it includes `evidenceSource=screenshotVisual`, screenshot evidence, `visualObservation`, and `domEvidenceGap`. Agent upload containment now uses `BLOCKED_NEEDS_VISUAL_REVIEW` for visual-fallback gate failures instead of generic containment. This remains review-only evidence, not automatic screenshot PASS.
- P0.25: address the first save/reopen/row-download flow slice. Status 2026-05-20: implemented as `npm run verify:p0-save-reopen-download-flow` plus navigation/capability regressions. The fix targets shared official UI readiness/navigation behind `F-02`, `F-04`, `G-02`, and `M-07`: official empty project pages with `新增自訂報表` + `無數據` are now valid report-list pages for create-report, official `/report/new` is treated as an editor route during back-to-list readiness, and sidebar navigation no longer confuses editor `拼貼模式` with left-nav `拼貼報表`. This is a helper-runtime compatibility slice; `K-09`, `L-08`, `M-06`, `M-09`, `M-10`, and `M-11` still need dedicated action-template slices.
- P0.26: field picker / 30-day date preset action-template slice. Status 2026-05-20: implemented as `npm run verify:p0-field-picker-date-preset-templates` plus runtime/evidence regressions. `case-scope-runtime-contracts.json` now extends from 15 reduced-smoke contracts to 17 by adding `BIUI_COLLAGE_R001-K-09` and `BIUI_COLLAGE_R001-L-08`. `K-09` routes to `collage.observeFrontendState(fieldPicker)` and collects row-scoped `fieldPicker.signature` + `fieldControl.after` evidence for selecting `新增帳號數` after `每日報表`. `L-08` stays `datePanel` but now executes both `dateRange.preset.past30Days` and `dateRange.preset.recent30Days`, recording per-preset before/after date button text/state. This closes only those two evidence gaps; `M-06`, `M-09/M-10`, and `M-11` remain separate P0 action-template slices.
- P0.27: report mutation flow action-template slice. Status 2026-05-20: implemented as `npm run verify:p0-report-mutation-templates` plus runtime/vocabulary/result-contract regressions. `case-scope-runtime-contracts.json` now covers 21 contracts by adding `BIUI_COLLAGE_R001-M-06`, `M-09`, `M-10`, and `M-11` under `report_mutation_flow`. `M-06` and `M-09` use safe `collage.observeFrontendState` flows for save-modal cancel and copy-modal cancel evidence; `M-10` uses `collage.copyReportAndVerify` to create a copied report and verify a project-list row; `M-11` uses `collage.updateExistingReportAndReopen` to update settings, return to the list, reopen the report, and verify persisted state. Domain pack data now names the save/copy modal controls, update button, report row, and reopen persistence evidence. This closes the remaining P0.23 report mutation template gap as a compatibility bridge; the final Gen4 interpreter should still execute these from declarative action contracts instead of hardwired helper branches.
- P0.28: post-P0.27 reduced blocked-smoke stabilization. Status 2026-05-20: implemented first stabilization slice after reduced run `12594ce2-1555-432d-800c-3663934913a1`. Platform runtime now detects helper pre-run browser/page/context closed blockers, recreates the same-case browser lease once, retries helper pre-run, and if the retry still reports a lifecycle failure writes trusted single-case `BLOCKED / BLOCKED_BROWSER_SESSION_CLOSED` so later cases continue instead of leaving `PENDING`. Date and mapping fixes were also added where smokeable: relative dynamic date input no longer depends only on brittle `#startDayInput/#endDayInput`; `L-05` now has a structured `datePanel` contract for `上週 -> 本週`; `D-01` parses `來源報表(多源)` into concrete source reports instead of treating `(多源)` as a literal UI option. Follow-up after run `a18ad789-2ace-442a-a161-e1769bcc6e1e`: added `npm run verify:agent-runtime-file-access` because the dev service hit local filesystem `EPERM` loading helper executor/readable-stream before any trusted case judgment. This smoke is deployment hygiene, not product judgment logic. Verification entrypoints: `npm run verify:agent-runtime-file-access`, `npm run verify:p0-helper-browser-session-containment`, `npm run verify:p0-live-action-templates`, `npm run verify:bi-official-ui-object-vocabulary`, `npm run verify:official-observation-contract`, `npm run verify:result-evidence-gate`, `npm run verify:agent-result-contract`, `npm run verify:p0-live-run-packet-regression`, `npm run verify:p0-108-blocked-triage`, `npm run typecheck --prefix agent`, and `npm run build --prefix agent`. Remaining blockers are intentionally not hidden: `I-04/J-10` project-create modal flow, `J-03/J-08` project-page visual/workflow assertions, `F-02` save/reopen evidence chain, and `J-12` five-project precondition builder still require separate reduced-smoke slices before full 108-case green-light.
- P0.29: BI official inline formula-builder domain/testcase alignment. Status 2026-05-20: implemented after Tommy provided the official E-group screenshots for calculated-field setup. The BI domain pack now models the real inline formula builder rather than the stale formula-modal assumption: `formulaEditor.inlineExpressionArea`, `formulaEditor.keypad`, `formulaEditor.insertFieldButton`, `formulaEditor.sourceReportPicker`, `formulaEditor.fieldPicker`, operator/number/clear/delete buttons, and `action-contracts/setCalculatedFormula.json`. `ui-contract.json`, `ui-object-vocabulary.json`, `component-inventory.json`, `evidence-schema.json`, `lint-rules.json`, and `discovery/visual-alignment.json` all record the boundary that trusted product judgment requires visible token/keypad interactions; `direct_fill_inline` / DOM mutation / internal JS setter can only support `BLOCKED_TOOL_LIMITATION`, not PASS/FAIL. Testcase package v1.13 updates `E-01` to `E-04` with `baseFields[]`, `formulaSteps[]`, structured steps, and `Vocabulary Contract` rows. `E-03` now uses `退費追蹤 / 退費總金額` as the divisor source based on metadata and Tommy screenshots. Verification: `npm run verify:domain-pack -- --name BI_OFFICIAL_UI_COLLAGE`, `npm run verify:bi-official-ui-object-vocabulary`, and package consistency confirm E-01~E-04 have zero remaining warnings. This is domain/testcase contract alignment, not a platform runtime patch and not a BI-only helper.
- P0.30: shared lifecycle action contract layer. Status 2026-05-20: implemented as domain-pack contract/data preparation after reduced run `b9b0a41d-e8a6-4423-afd2-28a6bd158688` still had 10 BLOCKED concentrated in repeated flows. The domain authoring templates now require each new domain pack to identify cross-case lifecycle journeys such as create -> modal -> cancel/save, save -> project list -> find row -> reopen, copy -> save -> verify row, delete -> confirm -> cancel, picker/date preset -> apply -> verify state, and download/export -> artifact/toast/result. `BI_OFFICIAL_UI_COLLAGE` adds `action-contracts/reportLifecycle.json`, `projectLifecycle.json`, `projectList.json`, and `dateRangePanel.json`, with new evidence objects including `projectList.readiness.state`, `projectToolbar.selectionFlow.state`, `deleteConfirmModal.state`, `projectList.deleteCancelFlow.state`, `projectCreateModal.flow.state`, `dateRange.button.state`, `dateRange.applyPreset.state`, and `interactionLog`. `lint-rules.json` now warns when report/project/date/delete cases are authored without the shared lifecycle contract and row/read-back evidence. Verification entrypoint: `npm run verify:shared-lifecycle-action-contracts`, plus domain-pack and BI object-vocabulary verification. This is the declarative contract layer for the remaining F/G/I/J/L/M blockers; lowering live BLOCKED still requires the next runtime wiring slice to execute these contracts instead of leaving them as documentation.
- P0.30 runtime wiring: Status 2026-05-20: implemented in dev commit `8cdd5c8` as a Gen1/Gen2 compatibility bridge that executes the shared lifecycle contracts for the next reduced-smoke slice. `case-scope-runtime-contracts.json` now covers 29 structured contracts, adding/repairing `F-02`, `I-04`, `J-03`, `J-08`, `J-10`, `L-10`, and `M-07`. Planner/capability gate/helper executor now support `projectCreateModal`, `deleteCancelFlow`, project toolbar selection flow, save/list/reopen report lifecycle, and from-date preset observation. The executor records concrete interaction evidence (`projectCreateModal.flow.state`, `projectToolbar.selectionFlow.state`, `projectList.deleteCancelFlow.state`) and keeps screenshot as supporting evidence only. Verification: `npm run verify:domain-pack -- --name BI_OFFICIAL_UI_COLLAGE`, `npm run verify:official-observation-contract`, `npm run verify:p0-runtime-wiring`, `npm run verify:p0-live-action-templates`, `npm run verify:bi-official-ui-object-vocabulary`, `npm run verify:shared-lifecycle-action-contracts`, `npm run typecheck`, and `npm run build --prefix agent`. This still is not the final Gen4 action interpreter; it proves the contract shape through targeted runtime bridges before promoting the pattern into the stable core.
- P0.31: result evidence gate / run lifecycle isolation fix. Status 2026-05-20: implemented after reduced v1.14 run `5dbc8fa5-1843-4980-9729-3bd7ac4136b7` stopped at `BIUI_COLLAGE_R001-L-05`. The root cause was platform-level: result gate treated ordinary UI interaction booleans (`clicked=false`, `confirmed=false`) as if Codex claimed native confirm handling, raised `TOOL_BRIDGE_RESPONSE_MISSING`, and server terminalized the run before Agent-side upload containment could retry. `result-evidence-gate` now masks these generic interaction booleans, and result upload no longer terminalizes the run on `ResultEvidenceGateError`; it returns 422 for Agent containment/retry while non-gate ingest errors still fail the run. Verification: `npm run verify:result-evidence-gate`, `npm run verify:result-evidence-upload-containment`, `npm run verify:zombie-run-guards`, `npm run typecheck`, `npm run build`, and `npm run build --prefix agent`. This is platform evidence/lifecycle hardening, not BI-specific testcase customization.
- P0.32: Agent control WebSocket active-run resilience fix. Status 2026-05-21: implemented after reduced run `ff1fccf2-9350-4187-8837-42c808fe631d` failed at `BIUI_COLLAGE_R001-J-12` even though dev server was already on `8f513ed` and helper pre-run had produced J-12 frontend observation evidence. The failure was platform-level: a transient Agent control WebSocket `1006` / heartbeat timeout caused the CLI to cancel the active Codex turn with `CODEX_RUN_CANCELLED reason=agent_connection_closed`. The Agent now treats connection close/error as diagnostic continuation while an active local task is running, queues outbound events for reconnect, and only cancels for explicit `task.cancel` or process stop signals. `npm run verify:zombie-run-guards` now forbids the old connection-loss cancel path. Verification: `npm run verify:zombie-run-guards`, `npm run typecheck`, `npm run build --prefix agent`, `npm run build`, `npm run verify:agent-runtime-file-access`, and `npm run verify:agent-context-prep`. This is platform run lifecycle resilience, not BI domain/testcase customization and not the final Gen4 interpreter.
- P0.33: reduced-smoke live blocker compatibility bridge. Status 2026-05-21: implemented after reduced run `10e944dc-4d9b-4e32-b197-9a8339dd52e5` completed without run interruption but still produced excess BLOCKED evidence. The slice handles four smokeable gaps without making Tommy's oracle permanent product truth: preset date cases such as `B-05` may pass result-contract evidence when the requested preset label is visibly applied even if the computed represented range is not directly visible; project-list delete cancel cases such as `J-08` can recover report rows from current page body text when DOM row extraction returns zero; from-date preset cases such as `L-10` now map `dateRange.preset.fromDateToYesterday` / `fromDateToToday`, fill a start date, and record interaction outcomes; report mutation planning keeps `M-11` on `updateExistingReportAndReopen` instead of regressing to open-only. `L-05`-style failed preset switches now expose `actualOutcome` and `recommendedFailureClassification` so result judgment can distinguish product interaction failure from generic evidence insufficiency. Verification: `npm run verify:p0-33-live-blocker-regressions`, `npm run verify:official-observation-contract`, `npm run verify:p0-runtime-wiring`, `npm run verify:bi-official-ui-object-vocabulary`, `npm run verify:shared-lifecycle-action-contracts`, `npm run verify:agent-result-contract`, `npm run verify:result-evidence-gate`, `npm run verify:date-ui-evidence`, `npm run verify:p0-live-action-templates`, `npm run typecheck`, `npm run build --prefix agent`, and `npm run build`. This remains a Gen1/Gen2 runtime bridge consuming domain/action contracts, not a BI-only helper and not the final generic Gen4 interpreter.
- P0.34: Codex no-result containment after current-run helper evidence. Status 2026-05-21: implemented after reduced run `071a690a-314e-4d93-8589-84772c359d6e` reached `BIUI_COLLAGE_R001-G-05`, produced helper pre-run evidence, then Codex ended without a trusted `output/result.xlsx` because browser preflight/tooling was unavailable. The platform now treats this as process isolation: when Codex exits cleanly but no workbook exists, package/document consistency has no error, and helper pre-run has current-case evidence, Agent writes a trusted single-case `BLOCKED / CODEX_NO_RESULT_XLSX` containment workbook before upload so later cases continue. This is not a product PASS/FAIL judgment and does not hide missing helper evidence: no current-run helper evidence or consistency errors still stop for PM/runtime review. Verification: `npm run verify:codex-no-result-containment`, `npm run verify:runtime-containment-result`, `npm run verify:p0-helper-browser-session-containment`, plus type/build checks. This is platform runtime containment, not BI testcase customization.
- P0.35: server-side agent disconnect policy alignment. Status 2026-05-21: implemented after run `0c1ce924-a4bd-410a-be99-0f880cd525be` was cancelled by `remote_run_failed`. Root cause was a policy mismatch: P0.32 made the Mac Agent keep local active tasks alive across control WebSocket loss, but the server still terminalized any busy-agent disconnect as `FAILED`; on reconnect the server saw a terminal run snapshot and sent `task.cancel reason=remote_run_failed`, killing the still-running local task. Server-side `agent_lost` now records a non-terminal interruption event/log with `terminalized=false` and keeps the run `RUNNING`, so reconnecting Agents can continue and later upload trusted results or containment workbooks. Verification: `npm run verify:agent-roundtrip` now asserts both explicit disconnect and heartbeat timeout do not terminalize active runs.
- P0.37: reduced-smoke blocked judgment/order bridge. Status 2026-05-22: implemented after run `3fe0f31b-a0ca-47f7-9712-925531fcc8c0` reduced smoke still left six BLOCKED cases. The slice separates four concerns: result-contract now treats static-tab flow as required only when explicitly under test and ignores collage no-display false contradictions; upload containment reruns deterministic enrichment before retry upload; frontend observation evidence can promote deterministic `FAIL_INTERACTION_FAILED` from BLOCKED to FAIL; BI domain pack adds `saveReportDisabled` / `editorToolbar.saveButton.state` and routing keeps `report_mutation_flow` ahead of generic open-report-only flows. Verification: `npm run verify:p0-33-live-blocker-regressions`, `npm run verify:agent-result-contract`, `npm run verify:p0-report-mutation-templates`, `npm run verify:official-observation-contract`, `npm run verify:p0-scope-contract`, `npm run verify:bi-official-ui-object-vocabulary`, `npm run verify:p0-live-action-templates`, typechecks, and builds. This remains a Gen1/Gen2 compatibility bridge consuming domain contracts, not the final generic Gen4 interpreter.
- P0.38: final reduced-smoke blocked bridge. Status 2026-05-22: implemented after run `f5196a8d-edb0-42e5-830f-3ef9efc99bec` completed with only `B-05` and `M-11` still BLOCKED. `B-05` exposed a platform browser-session lifecycle classification gap: `BROWSER_SESSION_LEASE_MISSING` now triggers the existing same-case lease recovery/retry path and, if still failing, single-case `BLOCKED_BROWSER_SESSION_CLOSED` containment. `M-11` exposed a report mutation judgment gap: `update-reopen-evidence.json` can now promote BLOCKED to `FAIL / FAIL_INTERACTION_FAILED` when a valid editor change was applied but the visible `更新設定` action was not clicked. The executor no longer attempts reopen after update-not-clicked and no longer marks non-reopened editor state as persisted. Verification: `npm run verify:p0-helper-browser-session-containment`, `npm run verify:p0-33-live-blocker-regressions`, `npm run verify:agent-result-contract`, `npm run verify:p0-live-action-templates`, typechecks, and builds. This remains a Gen1/Gen2 bridge: platform retry classification stays generic; report mutation semantics remain evidence-contract driven, not oracle hard-code.

- P0.40: full-ready package cleanup after reduced smoke. Status 2026-05-27: generated `BIUI_COLLAGE_R001` v1.14 full-ready package with 95 active cases, removing `J-12` project-limit setup and `L-10` non-visible from-date preset from the main full run. `L-02` now asserts only currently visible date presets; `A-06` uses the UI-current field list rather than metadata expected count for judgment; `I-03` no longer treats project count >=5 as a sidebar observation blocker; `J-10` declares the <=4 project-count precondition. The BI domain pack now marks `dateRange.preset.fromDateToYesterday` and `dateRange.preset.fromDateToToday` as not single visible presets in the current official UI and adds a lint warning for future authoring. Authoring docs now require environment-sensitive upper-limit cases to be split into focused packages and require visible inventory assertions to be backed by UI/screenshots/visual-alignment evidence. This is domain/testcase contract cleanup, not platform runtime or a new BI-only helper.
- P0.55: evidence-driven false blocker bridge for the 95-case full run. Status 2026-06-16: implemented after run `29c5ccd0-8027-46bf-8221-0d5ca0736065` showed remaining blockers concentrated in helper evidence quality rather than product-wide failure. Platform runtime now fail-fast blocks calculated-metric preview when date setting fails, so formula cases do not continue into polluted preview requests. Temporary report delete evidence now requires a completed visible delete confirmation before treating row persistence as a product deletion failure. The BI official domain pack adds `dateRangeLimit` observation, `dateRange.limitValidation` object/evidence, and an L-11 case-scope contract for 91-day blocked / 90-day accepted validation without preview. The structured contract loader now merges input snapshot contracts with repo domain-pack contracts, using input as precedence and repo as missing-case fallback. Verification: `npm run typecheck`, `npm run verify:domain-pack`, `npm run verify:official-observation-contract`, `npm run verify:p0-runtime-wiring`, `npm run verify:p0-live-action-templates`, `npm run verify:p0-live-run-packet-regression`, `npm run verify:p0-scope-contract`, `npm run verify:p0-offline-route-judgment`, `npm run verify:p0-report-mutation-templates`, and `npm run verify:p0-33-live-blocker-regressions`. This is still a Gen1/Gen2 compatibility bridge: roughly 55% platform shared runtime, 40% domain-pack/action-contract data, and 5% targeted case-scope/smoke guardrails; it does not make Tommy's manual oracle product truth and does not replace the later Gen4 declarative interpreter.
- P0.56: R091 false blocker route/judgment repair. Status 2026-06-18: implemented after run `43817969-b39b-407b-842e-efc0ee383319` showed remaining blockers where evidence did not support a product-wide regression. The BI official domain pack now includes `metricRowControls` observation semantics and a K-04 case-scope contract for first metric-row delete/duplicate controls. M-03/M-04/M-05 now have report-mutation/save-modal contracts and reuse the same configureMetric + preview precondition as M-06 before observing the save modal. Platform runtime now judges `dateRangeLimit` by whether the invalid 91-day range was actually applied and whether a 90-day warning was visible, rather than by helper operation success alone. Modal observation now tolerates official UI implementations that expose modal text/inputs without standard dialog wrappers. Regression smoke no longer protects K-04 screenshot-only project-row tooltip fallback; it requires K-04 to route to `metricRows.deleteRowButton` and `metricRows.duplicateRowButton`. Verification: `npm run verify:p0-runtime-wiring`, `npm run verify:p0-33-live-blocker-regressions`, `npm run verify:p0-live-action-templates`, `npm run verify:official-observation-contract`, `npm run verify:p0-live-run-packet-regression`, `npm run verify:p0-offline-route-judgment`, and `npm run build`. Layer split is roughly 45% platform runtime, 45% BI domain-pack/action-contract data, and 10% regression fixture guardrails; this remains a compatibility bridge, not the final Gen4 declarative action interpreter.
- P0.57: R093 prod-baseline evidence triage and contract wiring repair. Status 2026-06-22: implemented after run `c2009a71-1b20-470b-9920-cf834be34ca6` under the explicit premise that production is the correctness baseline and dev is a speed/stability optimization branch. The repair does not classify high BLOCKED as product signal by itself. It adds platform vocabulary `unselect` / `unselected`, BI official objects for toolbar unselect and project-list pagination, J-04/J-15 case-scope runtime contracts, and executor support for `projectToolbar.selectionToggle.state` plus `projectList.pagination.state`. M-03/M-04/M-05 capability gate and helper plan now agree on save-modal mutation preconditions. J-09 official delete report cases use the safe temporary delete helper, while frontend observation contracts such as J-07 keep precedence over generic delete regexes. Verification: `npm run verify:p0-runtime-wiring`, `npm run verify:capability-gate`, `npm run verify:official-observation-contract`, `npm run verify:p0-report-mutation-templates`, `npm run verify:p0-live-action-templates`, `npm run verify:p0-offline-route-judgment`, `npm run verify:p0-live-run-packet-regression`, `npm run verify:p0-33-live-blocker-regressions`, `npm run verify:domain-pack`, `npm run typecheck`, and `npm run build --prefix agent`. Remaining `D-01/E-03/K-02/J-14` style items are intentionally left as domain/testcase/spec/action-contract gaps rather than silent product fixes. This remains a Gen1/Gen2 bridge consuming domain contracts; it should later collapse into the generic Gen4 action interpreter.
