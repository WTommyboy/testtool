...
- 背景：run `3fe0f31b-a0ca-47f7-9712-925531fcc8c0` 沒有 run lifecycle 中斷，但仍有 `BLOCKED=6/16`。逐題拆解後不是單一 helper bug：`F-02/M-07` 是 result-contract 對拼貼模式「顯示方式不適用」與 static-date outcome 的檢查過嚴；`J-03` 是 upload containment 先把 workbook 降成 BLOCKED 後沒有再次 deterministic re-enrichment；`L-05` helper 已產生 `FAIL_INTERACTION_FAILED` 卻沒被轉成 FAIL；`M-01` 缺「儲存報表按鈕 disabled」domain observation route；`M-11` 的 `report_mutation_flow` 被泛用 open-report route 吃掉，只跑了 open project/report。
- Result contract：static tab flow 現在只在 case scope 明確把 static tab/date panel clickability 標為 `under_test` 時才是 PASS 必要條件；若 case 目標是保存/重開後 outcome correctness，且 evidence 已顯示 date range 或 preview/reopen 結果正確，不能因 `static tab clickability` 或拼貼模式沒有 display mode 而誤判 BLOCKED。`display=not_applicable_collage_no_display_mode` 會被視為非矛盾 evidence；reopen report 的 visible date text 也可抵銷舊 helper report 的 `dateRange=false`。
- Result evidence enrichment：BLOCKED rows 目前支援兩種 deterministic re-judgment：一是 current-run helper evidence 足夠時升 PASS；二是 frontend observation evidence 明確給出 `recommendedFailureClassification=FAIL_INTERACTION_FAILED` 時改判 FAIL，並保留前一版 detail_json 與 current-run evidence source。Upload containment 後會再跑一次 deterministic enrichment，再重跑 workbook self-check，避免 `J-03` 這類先 containment 後可判 PASS 的 case 被鎖成 BLOCKED。
- Domain/runtime route：BI domain pack 補 `saveReportDisabled` observation contract，對應 `editorToolbar.saveButton.state` evidence，不再把 `M-01` 誤導到 row delete tooltip route。Planner 對 `routeIntent=report_mutation_flow` 的 case 不再先套泛用 open-report-only route，讓 `M-11` 保持 `openProject -> openReportFromProjectList -> updateExistingReportAndReopen`。
- 邊界：這仍是 P0 Gen1/Gen2 compatibility bridge。平台層修的是 judgment/order/routing guard；BI-specific 的 toolbar/save button/date panel 語意只放在 BI domain pack。這不是把 Tommy 的 3fe0f31b oracle 寫死，也不是每個 domain 都新增小 helper。
- 驗證：`npm run verify:p0-33-live-blocker-regressions` 覆蓋 `M-01` save button disabled route、`M-11` report mutation route priority、`L-05` BLOCKED→FAIL deterministic classification，以及 `B-05/J-03` deterministic PASS path。另回歸 `npm run verify:agent-result-contract`、`npm run verify:p0-report-mutation-templates`、`npm run verify:official-observation-contract`、`npm run verify:p0-scope-contract`、`npm run verify:bi-official-ui-object-vocabulary`、`npm run verify:p0-live-action-templates`、`npm run typecheck --prefix agent`、`npm run typecheck`、`npm run build`、`npm run build --prefix agent`。

### 2026-05-22 - P0.38 final reduced-smoke blocked bridge

- 背景：run `f5196a8d-edb0-42e5-830f-3ef9efc99bec` 已完整跑完，reduced smoke 的 BLOCKED 降到 2 題。剩餘兩題不是同類問題：`B-05` 是 helper pre-run 第一題遇到 `BROWSER_SESSION_LEASE_MISSING`，舊 retry/containment 只辨識 target/page/context closed 與 stale；`M-11` 則已有 current-run evidence 顯示日期已改成「昨日」，但 `更新設定` 按鈕沒有被點擊，舊 judgment 把它留成 `EVIDENCE_INSUFFICIENT`。
- Runtime fix：`BROWSER_SESSION_LEASE_MISSING` 納入 browser-session lifecycle 分類。下一次同類 helper pre-run 會先走既有 same-case browser lease recovery + retry；若 retry 仍失敗，才寫單題 `BLOCKED_BROWSER_SESSION_CLOSED` containment，避免第一題直接變不可判而影響後續。
- Judgment fix：`result-evidence-enricher` 新增 `update-reopen-evidence.json` deterministic fail path。若 report mutation evidence 顯示 valid editor change 已套用、`更新設定` 可見但 `clicked=false`，Agent 會把原 BLOCKED 改判為 `FAIL / FAIL_INTERACTION_FAILED`，保留 previous detail 與 current-run evidence。這是依 interaction evidence 判產品流程失敗，不是人工 oracle hard-code。
- Executor fix：`updateExistingReportAndReopen` 不再於更新按鈕未點擊時硬走 back-to-list/reopen，也不再把未重開的 editor 狀態誤算成 `persisted=true`。未點擊更新時 evidence 會寫 `recommendedFailureClassification=FAIL_INTERACTION_FAILED` 與 `UPDATE_REOPEN_SKIPPED_AFTER_UPDATE_NOT_CLICKED`。
- 驗證：新增/更新 smoke 覆蓋 `BROWSER_SESSION_LEASE_MISSING`、`M-11 update-reopen-evidence BLOCKED→FAIL`，並用 f5196a8d 實際 result copy 確認 M-11 會被轉成 `FAIL_INTERACTION_FAILED`。已跑 `npm run verify:p0-helper-browser-session-containment`、`npm run verify:p0-33-live-blocker-regressions`、`npm run verify:agent-result-contract`、`npm run verify:p0-live-action-templates`、`npm run typecheck --prefix agent`、`npm run typecheck`、`npm run build --prefix agent`、`npm run build`。
- 邊界：B-05 修的是平台 browser-session retry 分類，不是產品判定；M-11 修的是 report mutation evidence judgment，不新增 domain 小 helper。這仍是 Gen1/Gen2 compatibility bridge，目標是讓 reduced smoke 剩餘 BLOCKED 轉為「可 retry/contain」或明確 PASS/FAIL。

### 2026-05-26 - P0.39 49022034 evidence-shape and lifecycle judgment bridge

- 背景：run `49022034-109f-44c0-9707-6b1867c21808` 已使用正確 domain pack，主要剩下結果形狀/流程證據判讀問題，而不是 108 題全面性 runtime 中斷。人工 review 指出需要同時看 `BLOCKED` 與 `FAILED`，且不能把「剛好相似」的判斷當成已符合規則。
- CSV/preview：`BIUI_COLLAGE_R001-F-07` 的 UI 下載 CSV 是「日期列 + 指標欄」，preview table 是「指標列 + 日期欄」矩陣。舊 comparator 逐列逐欄比對，因此誤判 FAIL；新版 `csv-preview-comparison` 會辨識 `csv-date-rows-vs-preview-metric-matrix`，用日期 key 對齊後比較值與區間總和。已用 49022034 實際 artifact smoke：62 個日期、CSV 欄位 `新增帳號數` 與 preview row `新增帳號數` 全部匹配，`rowCountMatchesPreview/allSeriesMatched/tableHeaderMatches/tableRowsMatched=true`。
- Modal/list lifecycle：`I-04` project-create modal 與 `J-08` delete-cancel flow 不再只依賴 dialog wrapper。若 open action 成功且接著完成 modal-only interaction（type/cancel 或 delete/cancel），observation evidence 會標記 `modalInteractionObserved`，避免 UI 沒有標準 dialog role 就誤 FAIL/BLOCKED。`J-08` 刪除目標選取也改為優先上方可見 row，並輸出 `rowBeforeCancel`，方便 result gate 判斷 cancel 後 row 數未變。
...
### 2026-06-04 - P0.48 production promote for P0.42-P0.47 runtime slice

- 決策：Tommy 判定目前 dev 版可先作為正確版推 production，速度優化另開後續工作。本次 promote 不包含後續速度優化，也不混入仍 dirty 的 authoring/domain-pack/RC verify 變更。
- Commit / branches：runtime commit `3c79ebb fix: harden BI runtime result handling` 已先推 `dev/uat-agent-config-isolation` 與 `codex/uat-tool-mvp`。因遠端 `refactor/mac-agent-mvp` 同時存在平行 prod-side commits `84371be` / `b9d2c60`，後續用 no-tree-change merge commit `05d6934 merge: preserve prod branch history` 保留 ancestry；`dev/uat-agent-config-isolation`、`refactor/mac-agent-mvp`、`codex/uat-tool-mvp` 目前皆指到 `05d6934`。
- Railway：production `/version` 與 `/health` 已確認 healthy。自動部署先顯示 runtime tree commit `3c79ebb`，後續刷新到 ancestry merge commit `05d6934`；兩者 tree 相同。後續 docs-only closeout commit 可能再推高 branch head，但不改 runtime 行為。
- 本機 production Agent：production launchd 仍使用 `/Users/tommy/Downloads/codex_galaxy/uat-tool`、`/Users/tommy/.uat-agent` 與 `com.tommy.uat-agent`。該 checkout 有既有 unrelated dirty docs，未做 destructive reset；改以 dev commit build 後同步 generated `agent/dist` 到 production checkout，再 `launchctl kickstart -k gui/$(id -u)/com.tommy.uat-agent` 重啟。重啟後 service running，pid `77258`，`node agent/dist/cli.js status` 顯示 `codex_model=(codex-cli-default)`。
- Local Codex config：production doctor 首次失敗是因 `/Users/tommy/.codex/config.toml` 的 `service_tier="priority"` 已不被 Codex CLI 0.125.0 接受。已改為 `service_tier="fast"`；`codex mcp list` 之後成功，doctor `ok=true`，`codex-model-config=PASS`，`playwright-mcp-availability=PASS`。
- Promote 前 smoke：已通過 `npm run verify:post-result-exit-policy`、`npm run verify:runtime-containment-result`、`npm run verify:agent-resume`、`npm run verify:agent-browser-session-isolation`、`npm run verify:p0-33-live-blocker-regressions`、`npm run verify:p0-visual-fallback-contract`、`npm run typecheck --prefix agent`、`npm run typecheck`、`npm run build --prefix agent`、`npm run build`、`git diff --check`。

### 2026-06-13 - UAT evidence review discipline rule

- 背景：run `6a23582b-f4dd-4525-b151-e98de16fd47b` 顯示 `PASS=55 / BLOCKED=40`，但逐題讀 report/archive 與 helper artifacts 後，主要原因分為 `CODEX_RUNTIME_RESULT_WRITE_FAILED`、helper route/domain/testcase 停點、以及 `BLOCKED_NEEDS_VISUAL_REVIEW`。只看 BLOCKED 比例會誤導成產品風險，Tommy 人工複核代表題也證明多數是 false blocked / tool limitation。
- 決策：把「UAT 結果判讀必須逐題 evidence triage」升級為常駐規則。收到 `UAT_report` / `UAT_archive` / result artifact 後，必須先逐題讀 fail category、detail_json、runtimeContainment、helper pre-run / continuation、helper artifact 的 `status` / `blockingReason` / `notReached` / DOM-network-preview-CSV-screenshot evidence，再分類成 product bug、runtime/lifecycle、helper route/planning、domain pack/contract gap、testcase flaw、tool limitation / visual judge gap、environment/auth/data issue。
- 修改檔案：`/Users/tommy/Downloads/codex_galaxy/AGENTS.md` 新增第 13 節「UAT 結果判讀紀律」；本 repo `AGENTS.md` 新增 `UAT Evidence Review Discipline`。
- 邊界：這是 rules / operating discipline 更新，不改 runtime、helper、domain pack、testcase、DB schema 或 deployment。後續仍建議把 triage 做成 script/verify，降低單靠模型遵守規則的風險。

### 2026-06-13 - P0.49 false BLOCKED reduction for run 6a23582b

- 背景：同一輪 `6a23582b-f4dd-4525-b151-e98de16fd47b` 中，Tommy 人工複核多個代表題正常。逐題讀 artifact 後，主要 false BLOCKED 來源不是產品行為，而是 Codex result write/thread persistence、helper route precondition、official UI icon-only/empty picker、save modal input discovery、editor/list download route 分流不足。
- Runtime：`CodexRunner.start` 預設以 `codex exec --ephemeral` 啟動新題，避免全新 `exec` turn 在本機/launchd 環境寫 rollout items 時出現 `thread ... not found`，污染成 `CODEX_RUNTIME_RESULT_WRITE_FAILED`。`resume()` 不加 `--ephemeral`，因 resume 仍需要既有 thread id。
- Helper：`BI_OFFICIAL_UI_COLLAGE` 補六個平台層修正：日期-only preview 在執行前用真實 UI 補預設 metric；temporary delete flow 將臨時報表名與 source report selection 分離；official field picker 在 source change 後若 options 暫空會重開重讀一次；save report helper 優先 modal-scoped text input；editor toolbar download 不再誤走 saved-report list CSV contract；report-list row download 點擊失敗後會繼續 row-action fallback。
- Dev Agent：本機 dev agent 已在 build 後以 `launchctl kickstart -k gui/$(id -u)/com.tommy.uat-agent-dev` 重啟；`launchctl print` 顯示 `state=running`、working directory `/Users/tommy/Downloads/codex_galaxy_dev/uat-tool`、pid `80606`。
- 邊界：沒有把 `退費總金額` 靜默對應到 iOS/Android/線下商城退費欄位；現有 `verify:helper-field-aliases` 明確要求 platform refund fields 不可滿足 aggregate `TOTAL_REFUND`，所以 E-03 仍屬 domain/testcase contract gap，需由 domain pack 或 testcase authoring 決定。
- 驗證：已通過 `npm run verify:p0-33-live-blocker-regressions`、`npm run typecheck --prefix agent`、`npm run verify:agent-resume`、`npm run verify:helper-field-aliases`、`npm run typecheck`、`npm run build --prefix agent`、`npm run build`、`npm run verify:runtime-containment-result`、`npm run verify:codex-no-result-containment`、`npm run verify:p0-visual-fallback-contract`、`npm run verify:p0-save-reopen-download-flow`、`node agent/dist/cli.js --config /Users/tommy/.uat-agent-dev/config.json doctor`、`git diff --check`。另手動 smoke 比對 `codex exec --json --ephemeral` 無 thread-not-found stderr，非 ephemeral `codex exec --json` 仍會輸出 rollout thread warning。

### 2026-06-13 - P0.50 R082 runtime regression repair

- 背景：NU_BI_p0_R082 / run `eda817a0-ff7d-4041-8433-d35236992a26` 表面 `BLOCKED=95/95`。逐題 evidence 顯示不是 BI 產品全壞，而是工具 runtime regression：`B-01` 起大量 `CODEX_RUNTIME_RESULT_WRITE_FAILED` 的 stderr 為 `unexpected argument '--ignore-user-config' found`；A 類與部分 helper artifact 另顯示 helper 連到錯誤 CDP endpoint `127.0.0.1:9222`，而 dev config 應為 `9223`。
- Root cause 1：P0.49 修改 `CodexRunner.start()` 時，只用 fake argv verify，未用真實 Codex CLI smoke。`CodexRunner.run()` 仍把 `-c service_tier` / `-c model_reasoning_effort` 放在 top-level，與 `exec --ignore-user-config` 混用後在 Codex CLI 0.125.0 解析失敗。修正後 `run()` 不再前置 `configArgs()`；`start()` / `resume()` 都把 `--ignore-user-config`、`-m`、`-c` 放在 `exec` 後面。
- Root cause 2：dev config 的 `codex_model` 空值仍可能讓 CLI 吃到本機 config 裡不支援的 `gpt-5.3-codex`。已用真實 smoke 確認 `gpt-5.4-mini` 可用，並將 `/Users/tommy/.uat-agent-dev/config.json` 設為 `codex_model="gpt-5.4-mini"`。
- Root cause 3：helper executor 子程序直接 `readConfig()`，在某些情境 fallback 到預設 debug port `9222`。修正後 helper executor 優先使用當前 run 的 `input/browser-session.json` lease endpoint，確保 helper 跟著 Agent 已建立的 browser session 走。
- Guardrail：新增 `npm run verify:codex-runner-real-smoke`，直接 instantiate `CodexRunner` 用真實 `codex exec` + `gpt-5.4-mini` 跑 `Reply with exactly OK.`。這個 smoke 是防止 argv/model regression 的必要 gate，不能再只靠 fake argv。
- 驗證：已通過 `npm run verify:codex-runner-real-smoke`、compiled `agent/dist/codex-runner.js` real smoke、`npm run verify:agent-resume`、`npm run verify:p0-33-live-blocker-regressions`、`npm run typecheck --prefix agent`、`npm run typecheck`、`npm run build --prefix agent`、`npm run build`、`node agent/dist/cli.js --config /Users/tommy/.uat-agent-dev/config.json doctor`、`git diff --check`。Dev doctor 顯示 model `gpt-5.4-mini`、CDP endpoint `9223`、profile matched。Dev launchd 已 kickstart。
- R083 追認：NU_BI_p0_R083 / run `738e5ce2-fe8b-446c-a4e1-602680a839c2` 仍為 `PASS=25 / BLOCKED=70`，archive 仍大量出現 `CODEX_RUNTIME_RESULT_WRITE_FAILED` 與 stderr `unexpected argument '--ignore-user-config' found`。該 run 執行時間為 `2026-06-13T12:07:23Z ~ 13:12:23Z`（台北 `20:07 ~ 21:12`），而修正後 compiled dist 在台北 `21:46` 才重建，因此 R083 未測到最新編譯版。R083 中的 25 個 PASS 來自 current-run deterministic helper evidence promotion，剩餘 BLOCKED 不應直接解讀為 BI 產品 regression。
- Dev agent 現況：`npm run build --prefix agent` 後 `agent/dist/codex-runner.js` / `agent/dist/bi-ui-helper-executor.js` mtime 為台北 `2026-06-13T21:46:24+0800`；已再次 `launchctl kickstart -k gui/$(id -u)/com.tommy.uat-agent-dev`，`launchctl print` 顯示 `state=running`、pid `47354`、working directory `/Users/tommy/Downloads/codex_galaxy_dev/uat-tool`、`UAT_AGENT_CHROME_DEBUG_PORT=9223`。

### 2026-06-13 - P0.51 pre-fix baseline smoke discipline

- 背景：R082/R083 修復過程暴露流程缺口：修復前沒有先對當下 running dev agent 做乾淨 pre-fix baseline smoke，只用 archive evidence 定位後直接修，再用 post-fix smoke 驗證。這讓「source 已修」與「compiled dist / launchd service 尚未吃到修正」之間的時間差到 R083 後才被明確抓出。
- 決策：把 pre-fix baseline smoke 升級成常駐工程規則。任何 runtime、Agent、CodexRunner、helper、browser session、model config、result write/upload、planner route、deployment 或 dev/prod agent 修復前，必須先針對目前 running artifact 做最小可重現 smoke，或明確記錄為何不能做；修後再跑 matching post-fix smoke，並確認 `dist` / launchd / Railway 等 running artifact 已更新。
- 修改檔案：`/Users/tommy/Downloads/codex_galaxy/AGENTS.md` 新增第 14 節「uat-tool 修復前 smoke 紀律」；本 repo `AGENTS.md` 的 Pre-Commit / Pre-Deploy Verification 新增 pre-fix baseline smoke 要求。
- 邊界：這次是流程/規則 guardrail，不改 runtime 行為、不改 BI 產品、不改 testcase/domain pack。後續修任何 UAT tool 問題時，回報必須分開列出 `pre-fix baseline`、`change`、`post-fix smoke`、`running artifact status`；若缺任一項，必須明說缺口。

### 2026-06-14 - P0.52 dev agent singleton and config reload hardening

- 背景：NU_BI_p0_R084 / run `76ca2b2f-7fd4-484a-863d-ff456651a244` 仍為 `PASS=25 / BLOCKED=70`。逐題 evidence 顯示不是 BI 產品 regression：25 題 PASS 來自 deterministic helper evidence promotion；大量 BLOCKED 的 `runtimeContainment.codexFailure.stderr` 仍是 `unexpected argument '--ignore-user-config' found`，且 `input/codex-context.json` 顯示 `codex_model="" / (codex-cli-default)`。這與當前 dev config 的 `codex_model="gpt-5.4-mini"` 不一致。
- Root cause：本機同時存在兩個 dev agent process。正式 launchd process 是 `/Users/tommy/Downloads/codex_galaxy_dev/uat-tool/agent/dist/cli.js --config /Users/tommy/.uat-agent-dev/config.json start`，但另有 Thu 11AM 啟動的舊 process `node agent/dist/cli.js --config /Users/tommy/.uat-agent-dev/config.json start --help`。舊 process 連同一個 dev token / config，會和正式 process 搶 Railway dev task；R084 實際 artifact 符合舊 process cached empty model 與舊 Codex argv path。
- Immediate fix：已強制終止舊 dev process，只保留 launchd dev agent。沒有動 production agent pid，也沒有推 prod。
- Runtime guardrail：新增 `agent/src/process-lock.ts`。`uat-agent start` 會在 config home 建立 `agent-process.lock/owner.json`，同一 config 第二個 `start` 會立即失敗 `AGENT_ALREADY_RUNNING`；stale lock 若 owner pid 已不存在會自動清理。
- Doctor guardrail：`runDoctor` 新增 `agent-singleton-process` check，掃描同一 config 的 running `node ... agent/dist/cli.js --config ... start` process；`count > 1` 時 doctor FAIL，避免重複 agent 仍被視為可測。
- Config guardrail：`cli start` 在每次 `task.dispatch` / `tool_response` / `browser.open_url` 前重新 `readConfig(configPath)`，避免長駐 process 因早期 cached config 持續使用舊 `codex_model` / runtime settings。啟動 log 也會輸出 pid、configPath、model、ignore-user-config、tier、reasoning，方便從 launchd log 確認 running artifact。
- 驗證：已通過 `npm run verify:agent-process-lock`、`npm run verify:agent-resume`、`npm run verify:p0-33-live-blocker-regressions`、`npm run verify:codex-runner-real-smoke`、`npm run typecheck --prefix agent`、`npm run typecheck`、`npm run build --prefix agent`、`npm run build`。重啟 dev launchd 後 pid `86975`，`node agent/dist/cli.js --config /Users/tommy/.uat-agent-dev/config.json doctor` 回 `ok=true`、`agent-singleton-process=count 1`、model `gpt-5.4-mini`、Playwright MCP PASS。第二個 `node agent/dist/cli.js --config /Users/tommy/.uat-agent-dev/config.json start` 會 exit 1 並輸出 `AGENT_ALREADY_RUNNING pid=86975`。compiled `agent/dist/codex-runner.js` real smoke 回 `OK`。
- 判讀：R084 的 70 BLOCKED 應歸 `runtime/lifecycle`，不是產品 bug、domain pack 或 testcase 結論。下一輪若還出現 `CODEX_RUNTIME_RESULT_WRITE_FAILED`，先檢查 archive 的 `codex_model` 是否為 `gpt-5.4-mini`、launchd log 是否有 `task_config_loaded`，以及 doctor 的 `agent-singleton-process` 是否仍為 PASS。

### 2026-06-14 - P0.53 R086 result detail_json path repair

- 背景：NU_BI_p0_R086 / run `31185ab3-cb10-46f2-b8be-42527155ef3c` 不是前一輪的 duplicate dev agent、wrong model 或 Codex argv 問題。local run 顯示 `model=gpt-5.4-mini`、helper 使用 browser lease endpoint `127.0.0.1:9223`、doctor singleton 為 PASS。Report 只完成 9 題後剩 86 PENDING，是因 B-08 result upload 前 self-check 擋下整輪。
- 根因：B-08 的 `output/detail.json` 本身是合法 FAIL detail object，但 `output/result.xlsx` 的 `測試案例.詳細紀錄JSON` cell 被 Codex 寫成檔案路徑 `/Users/tommy/.uat-agent-dev/runs/.../output/detail.json`，不是 JSON object 字串。`validateResultWorkbookContract` 正確回 `RESULT_XLSX_DETAIL_JSON_INVALID`，但舊流程無法修復此可恢復 writer mistake，於是整輪 failed。
- Runtime：`agent/src/result-workbook-repair.ts` 新增 run-local detail_json path expansion。只在 Codex-generated result upload 前、只處理 expected current case、且只接受位於本輪 `runDir` 內的 `.json` 檔；檔案必須 parse 成 JSON object 才會寫回 workbook cell。run 目錄外路徑、非 JSON object 或不存在檔案都不展開，保留 self-check error。
- Task runner：`repairSingleCaseResultWorkbook` 呼叫補入 `runDir`。既有 legacy `群組ID` repair 保留；result-contract workbook 若只有 detail_json path 問題，也會在 self-check 前修復，不需要走泛用 containment。
- 邊界：這是 `runtime/result-contract` 修復，不是 BI 產品判定修正。B-08 helper evidence 仍需獨立判讀為 product/date UI issue、helper route gap 或 testcase coverage gap；本 patch 只確保 workbook contract 不會讓整輪直接 terminal failed。
- 驗證：新增 `verify:agent-result-contract` fixture，覆蓋 run-local detail_json path 可展開、runDir 外 path 不可展開。另用 R086 實際 `output/result.xlsx` 複製本做 smoke：修復前 `RESULT_XLSX_DETAIL_JSON_INVALID`，修復後 contract `ok`。已通過 `npm run verify:agent-result-contract`、`npm run typecheck`、`npm run build --prefix agent`、`npm run verify:p0-33-live-blocker-regressions`、`npm run verify:agent-process-lock`、`npm run verify:p0-runtime-wiring`、`npm run verify:runtime-containment-result`。

### 2026-06-14 - P0.54 R087 date route and project-toolbar batch download bridge

- 背景：NU_BI_p0_R087 / run `87874654-7d36-4c87-9009-fc30c3280c1d` 已完整跑完且整體改善為 `PASS=73 / FAIL=3 / BLOCKED=17 / PARTIAL=2`。逐題讀 report/archive 後，剩餘可平台修補的重點不是產品整體 regression，而是 `B-08` 半動態日期 routing、`E-01/E-02/E-04` 靜態日期 fallback，以及 `N-05` 專案頁批次下載被誤導到 row-local download。
- Pre-fix baseline：修前已跑 `verify:date-ui-evidence`、`verify:date-variants-evidence-summary`、`verify:helper-field-aliases`、`verify:p0-33-live-blocker-regressions`、`verify:p0-live-action-templates`、`verify:p0-save-reopen-download-flow`，確認既有 date/route smoke baseline 可重現。R087 archive 中 `N-05` helper pre-run 顯示 route 為 `collage.openProject + collage.downloadCsvAndComparePreview`，但 H2 title/trigger 是「從專案頁第一個可下載報表 row 觸發下載」，證明 batch toolbar 題被 row download helper 誤測。
- Runtime date：`agent/src/bi-ui-helper-executor.ts` 新增 text-label structured date parser，支援 `2026/03/25 > 1 天前`、`2026-03-25 ~ 1 天前`、`14 天前 ~ 1 天前`、`今天/昨日/明天` 等 static/relative endpoint 組合。`setDateRange` 會先把 hybrid label route 到 structured endpoint helper；static range inline fill 驗證失敗時會重開 date panel 走 calendar fallback，並保留 `inputAttemptFailed` / `calendarFallback` evidence。
- Planner route：`helper-execution-plan` 修正 `preview_execution` 被 frontend observation prelude 搶走的順序問題；B-08 類 hybrid date preview 會使用 `collage.runDateVariantsPreviewEvidence`。同時收窄 date-preview helper preference，避免 B-09 這種 ordinary static preview 被誤導到 date-variant helper。
- Project toolbar batch download：新增 `project_toolbar_batch` download scope。Planner 只有在 case prose/params 明確含多列、多筆、批次、batch 或 multi 且有 project toolbar/download context 時才啟用；executor 會用 visible UI 勾選多個 project-list row checkbox，再點 toolbar semantic first button download control。`J-02` 未勾選 disabled-state observation 維持 `collage.observeFrontendState`，不被 toolbar/download 文字誤導。
- 邊界：沒有修改 `BI_OFFICIAL_UI_COLLAGE` domain pack，沒有把 `D-01/E-03` 的 aggregate metric gap 用 alias 靜默補齊，也沒有把 screenshot-only visual fallback 轉 PASS。這批是平台 helper/route bridge；domain pack/testcase contract gap 仍需獨立處理。
- 驗證：已通過 `npm run verify:p0-33-live-blocker-regressions`、`npm run verify:official-observation-contract`、`npm run verify:p0-save-reopen-download-flow`、`npm run verify:date-ui-evidence`、`npm run verify:date-ui-dom-smoke`、`npm run verify:date-variants-evidence-summary`、`npm run verify:p0-live-action-templates`、`npm run verify:result-evidence-gate`、`npm run verify:result-evidence-upload-containment`、`npm run typecheck`、`npm run build --prefix agent`、`npm run build`。本批尚未跑真實 BI UI full live smoke，屬 route/offline/regression smoke；下一輪需看 archive 是否真的到達 date setup、toolbar selection 與 download event。

### 2026-06-16 - P0.55 R089 evidence-driven false blocker repair

- 背景：run `29c5ccd0-8027-46bf-8221-0d5ca0736065` 已達 `PASS=74 / FAIL=3 / BLOCKED=14 / PARTIAL=4`，但逐題讀 report/archive/helper artifacts 後，剩餘 blocker 主要不是產品整體 regression。`E-01/E-02/E-04` 是公式 helper 在日期設定失敗後仍繼續 preview，甚至讓 formula request 受日期文字污染；`G-03` helper 只點到刪除 trigger，沒有完成可見確認 modal/native confirm，卻留下容易被解讀成產品刪除失敗的 evidence；`L-11` 只有 generic frontend observation，未執行 91/90 天上限流程。
- Runtime：`configureCalculatedMetricAndPreview` 現在在 structured/static date 設定失敗時 fail-fast，寫出 `workflowStatus=blocked`、`failedSubcondition=date_range` 與 skipped preview evidence，不再按 preview 或產生公式被日期值污染的 request。`createAndDeleteTemporaryReport` 現在只在 row 消失、delete trigger clicked、且可見 delete confirm modal 已確認 clicked 時才視為刪除流程完成；若沒有確認 modal/native confirm evidence，輸出 `DELETE_TEMP_CONFIRM_NOT_COMPLETED`，交由 Codex 判為 tool/flow limitation，而不是產品刪除 bug。
- Domain pack / contract：`BI_OFFICIAL_UI_COLLAGE` 新增 `dateRangeLimit` observation type、`dateRange.limitValidation` object/evidence，以及 `BIUI_COLLAGE_R001-L-11` case-scope runtime contract。L-11 會走 `collage.observeFrontendState(dateRangeLimit)`，以 visible UI 設定 91 天 invalid range 與 90 天 valid range，並要求沒有 preview request。本調整屬 domain pack/action contract，不是單一 case prose 判斷。
- Platform contract loader：`structured-case-scope` 改為 input run-packet contracts 優先、repo domain pack 補缺口。舊 run packet 若已有非空 `input/domain_case_scope_contracts.json` 但缺新 case，不會再阻止 repo 內新增的 case-scope contract 生效。`verify:p0-live-run-packet-regression` 新增 partial input snapshot fallback fixture。
- Smoke / 驗證：修前先用 29c evidence 與 direct plan smoke 確認 `N-02/M-06/M-09` 已有合約、`L-11` 缺 contract；修後直接 plan smoke 確認舊 29c packet 上 `L-11` 也載入 `dateRangeLimit`。已通過 `npm run typecheck`、`npm run verify:domain-pack`、`npm run verify:official-observation-contract`、`npm run verify:p0-runtime-wiring`、`npm run verify:p0-live-action-templates`、`npm run verify:p0-live-run-packet-regression`、`npm run verify:p0-scope-contract`、`npm run verify:p0-offline-route-judgment`、`npm run verify:p0-report-mutation-templates`、`npm run verify:p0-33-live-blocker-regressions`。
- 邊界：本批仍未把 `D-01/E-03` aggregate metric gap 以 alias 靜默補齊，也沒有把 screenshot-only visual review 自動轉 PASS。整體是平台共用約 55%、domain pack/contract 約 40%、case-specific guard 約 5%；case-specific 部分只限 smoke fixture 與 G-03/L-11 明確 case contract，沒有寫產品資料值客製邏輯。

### 2026-06-18 - P0.56 R091 false blocker route/judgment repair

- 背景：run `43817969-b39b-407b-842e-efc0ee383319` 維持 `PASS=74 / FAIL=2 / BLOCKED=14 / PARTIAL=5`，但逐題 evidence 顯示 blocker composition 有改善空間。`L-11` artifact 已證明 91 天區間出現「不可超過 90 天」警告且日期按鈕未套用 invalid range，但 runtime 仍因 `setStructuredDateRange().ok === true` 誤寫 `DATE_RANGE_LIMIT_INVALID_RANGE_ACCEPTED`。`K-04` 實際跑成 project-list `rowDeleteTooltip`，而非測第一列 metric row 的「刪除列 disabled / 複製列 enabled」。`M-03/M-04/M-05` 缺 case-scope contract，導致 save modal 題走泛用 toolbar/project route；`M-09` copy modal body text 已出現，但官方 UI 沒有標準 dialog wrapper，工具誤判 modal 未開且讀不到 default name。
- Pre-fix baseline：修前先跑 `npm run verify:p0-runtime-wiring`、`npm run verify:p0-offline-route-judgment`、`npm run verify:official-observation-contract`、`npm run verify:p0-live-action-templates`，確認既有 smoke 全綠但未覆蓋 R091 的 K/M/L false blocker pattern。這次先補 regression，再改 runtime/domain contract。
- Runtime：`dateRangeLimit` 現在以「invalid date 是否真的套用 + warning 是否可見」判斷 91 天是否被接受；只要警告可見且日期未套用，即視為成功阻擋，不再把 helper 操作 ok 誤判成產品 FAIL。`readVisibleModalState` 也補 `modalLikeBody/pageInputs/pageButtons` fallback，讓 official UI 在沒有 `[role=dialog]` / `.modal` wrapper 時仍可辨識 save/copy modal 與 input default value。`saveModalCancel` 新增通用 `reportNameValidationMode` evidence，支援長名稱、特殊字元、儲存專案 selector 的安全取消觀察。
- Domain pack / contract：`BI_OFFICIAL_UI_COLLAGE` 新增 `metricRowControls` observation contract/object coverage，並補 `BIUI_COLLAGE_R001-K-04` case-scope contract，targets 固定為 `metricRows.deleteRowButton` 與 `metricRows.duplicateRowButton`。新增 `M-03/M-04/M-05` report mutation contracts，沿用 M-06 的 createReport -> configureMetric -> preview -> saveModalCancel 前置，避免 save button disabled 時誤測 modal。`saveModal.projectSelect` 加入 UI object vocabulary。
- Regression：`verify:p0-33-live-blocker-regressions` 移除舊的 K-04 screenshot-only visual fallback fixture，改為強制 K-04 route 到 `metricRowControls`，並檢查 M-03/M-04/M-05 必須先完成 report mutation preconditions 才 observe save modal。
- 驗證：已通過 `npm run verify:p0-runtime-wiring`、`npm run verify:p0-33-live-blocker-regressions`、`npm run verify:p0-live-action-templates`、`npm run verify:official-observation-contract`、`npm run verify:p0-live-run-packet-regression`、`npm run verify:p0-offline-route-judgment`、`npm run build`。
- 邊界：本批是 false blocker route/judgment 修正，不更動 BI 產品、不推 prod、不把 `D-01/E-03` aggregate metric gap 靜默補齊，也不把 `G-03` 未完成 delete confirm 的 evidence 改判成產品 bug。層級約為平台 runtime 45%、domain pack/contract 45%、regression fixture 10%。

### 2026-06-22 - P0.57 R093 prod-baseline evidence triage and contract wiring repair

- 背景：run `c2009a71-1b20-470b-9920-cf834be34ca6` 完整跑完且為 `PASS=82 / FAIL=1 / BLOCKED=9 / PARTIAL=3`。Tommy 明確校正判讀基調：production 已是線上正確基準，dev 版是針對 prod 正確但較慢的優化；因此本輪非 PASS 不能先推論為產品壞，必須逐題讀 report/archive/helper evidence 後分類成 runtime/route/domain/testcase/spec/tool limitation。
- Evidence triage：`L-11` 是 false product fail，artifact 已顯示 91 天 invalid range 被 warning 阻擋且日期未套用；現有 `invalidRangeAccepted = invalidResult.ok && invalidDateApplied && !invalidWarningVisible` 判斷正確。`B-10` 屬 date variant evidence gap/partial；`D-01/E-03` 仍是 aggregate field/domain pack 或 testcase contract gap，不做 fuzzy alias；`K-02` 是 spec/testcase 期待與 official UI disabled radio 現況的 gap；`J-14` 仍缺 project deletion helper/contract，不在本輪硬補；`G-03/J-09` 是 delete helper route/click evidence 問題，不可再描述成產品刪除失敗。
- Runtime / route：`capability-gate` 與 `helper-execution-plan` 將 `M-03/M-04/M-05` 納入 save-modal report mutation precondition path，和 M-06 一樣走 `openProject -> createReport -> configureMetric -> preview -> observeFrontendState(saveModalCancel)`，避免只有 capability gate 寫到、實際 plan 卻缺 save-modal path。官方 J-09 「刪除報表」改走安全 temporary report delete helper；同時讓 frontend-observation contract precedence 高於泛用 delete regex，避免 J-07 row tooltip 類題被誤導到 delete helper。
- Domain pack / contract：`BI_OFFICIAL_UI_COLLAGE` 新增 `BIUI_COLLAGE_R001-J-04` toolbar unselect contract 與 `BIUI_COLLAGE_R001-J-15` project-list pagination contract。J-04 會選取報表列、取消選取同列，再驗證 download/delete toolbar disabled；J-15 會讀總筆數文字、切換 page-size、點下一頁並記錄 pagination state。新增 evidence schema `projectToolbar.selectionToggle.state`、`projectList.pagination.state`，並補 object vocabulary `projectList.reportRow.unselect`、`projectToolbar.downloadDeleteButtons`、`projectList.pagination.totalRecordText`、`projectList.pagination.nextPage`。
- Platform vocabulary：新增 generic primitive action `unselect` 與 expected outcome `unselected`。這是平台層能力，避免把「取消勾選/取消選取」寫成 BI 客製 action；domain pack 只負責具體 row/toolbar/pagination object。
- Helper executor：`observeFrontendState(projectToolbar)` 支援 select -> unselect flow 並輸出 toolbar after-state；新增 `projectListPagination` observation，透過 visible UI 讀總筆數、打開 page-size、選其他 page-size、點下一頁。`clickReportListDeleteControl` 補 body-index click 後的 modal 檢查與座標 click fallback；若仍無 modal，保留精確 click-attempt evidence，而不是升級成產品 bug。
- Smoke / 驗證：本批先用 direct smoke 抓到 M-03/M-04/M-05 gate/plan mismatch，再改 runtime/domain。修後已通過 `npm run verify:p0-runtime-wiring`、`npm run verify:capability-gate`、`npm run verify:official-observation-contract`、`npm run verify:p0-report-mutation-templates`、`npm run verify:p0-live-action-templates`、`npm run verify:p0-offline-route-judgment`、`npm run verify:p0-live-run-packet-regression`、`npm run verify:p0-33-live-blocker-regressions`、`npm run verify:domain-pack`、`npm run typecheck`、`npm run build --prefix agent`。`verify:domain-pack` 只剩既有 generic `domain-packs/BI` 文件 warning，`BI_OFFICIAL_UI_COLLAGE` PASS。
- 邊界：本批不推 prod、不改 BI 產品、不把 prod 正常行為包裝成產品 bug，也不把 `D-01/E-03/K-02/J-14` 靜默改成 PASS。預期改善的是 M-03/M-04/M-05 save-modal evidence、J-04 toolbar unselect evidence、J-15 pagination evidence、J-09/G-03 delete helper click evidence品質；剩餘 domain/testcase/spec gap 仍需 PM/domain authoring 決策。

### 2026-06-26 - TAG_TOOL R006 auto-resume containment repair

- 背景：NU_TAG_p0_R0006 / run `28a4ebd1-ca04-4545-b596-63039deb2a68` 只跑到 `BIUI_TAG_R001-D-05` 就整輪 `FAILED`，後面 66 題 `PENDING`。逐讀 report/archive 後，主因不是 Galaxy 產品壞：D-05 先把「點新增打開新增標籤頁」誤分類為 `irreversible_operation`，Mac Agent auto-approved 後，Codex `thread/resume` 又回 `no rollout found for thread id ...`，接著 runtime containment 因 helper pre-run 被 `CAPABILITY_GATE_degraded` 跳過而拒絕寫單題 result，最後整輪被 `CODEX_RUN_FAILED` terminal。
- Runtime：`writeCodexRuntimeContainmentResultIfNeeded()` 不再只接受 helper pre-run evidence；若同一 case 已有 current-run Tool Bridge request/response 檔，或 Codex result events 內有同題 MCP tool call evidence，也可寫單題 `BLOCKED/CODEX_RUNTIME_RESULT_WRITE_FAILED` containment。沒有 helper / Tool Bridge / MCP evidence 時仍維持 `NO_CURRENT_RUN_HELPER_EVIDENCE_TO_CONTAIN`，避免無證據硬寫結果。
- Gate：runtime containment 現在同時檢查 `input/document-consistency.json` 與 `input/test-package-consistency.json`，任一為 error 都跳過 containment，保留 package/design error 給 PM 決策。
- Tool Bridge 邊界：補強 task prompt 與 `agent-skills/uat-tool/rules/tool-bridge.md`。打開新增/編輯/詳情/查看設置頁、展開下拉/選單/popover、切換分頁或每頁筆數、點只讀 row link 都不是不可逆操作；若 browser safety layer 擋住，應寫含 preflight evidence 的 `TOOL_EXECUTION_UNAVAILABLE` 或 `EVIDENCE_INSUFFICIENT`，不要要求不可逆授權。
- Follow-up audit：發現 `advance_case` 時只清 `tool-requests.json` / `tool-requests-resume.json`，未清 `tool-requests-auto-*` / `tool-responses-auto*`。雖然 runtime containment 會用 caseNo 比對降低誤用機率，但 task-runner policy scanner 的 `hasToolBridgeResponse()` 只看是否存在 auto response，可能被前一題 stale auto response 影響。已補 `removeStaleCaseOutput()` 清理 auto Tool Bridge request/response/policy-violation 檔，並在 `verify:tool-bridge` 加 regression。
