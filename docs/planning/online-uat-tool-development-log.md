...
## 2026-06-27 - TAG_TOOL R0007 platform/domain boundary and false bug containment
...
- Domain: `TAG_TOOL`
- Result summary: 14 PASS, 26 BLOCKED, 1 PARTIAL, 47 PENDING, run ended at `BIUI_TAG_R001-E-08`.

Interpretation:

- This was not evidence that the tag product suddenly broke. The run mixed platform lifecycle issues, immature TAG domain-pack/helper coverage, and missing test fixtures.
- BI official UI collage currently works better because its domain pack has mature UI objects, action contracts, helper routing, and evidence contracts. TAG_TOOL is a newer pack, so changing feature area exposes missing contracts instead of only product behavior.
- Any future report for TAG_TOOL must first classify failures by layer: platform lifecycle/result pipeline, domain pack/action contract gap, testcase/fixture gap, and only then product bug.

Platform findings:

- `BIUI_TAG_R001-E-08` reached a Tool Bridge request for removing an uploaded file chip. Mac Agent auto-approved it, then Codex resume failed with `thread/resume failed: no rollout found for thread id ...`.
- The real failure was a runtime lifecycle/containment problem: current-run Tool Bridge evidence existed, but runtime failure containment did not trust/write a single-case blocked result in that actual file shape.
- A local smoke copied the real E-08 Tool Bridge evidence shape (`tool-requests.json` + unsuffixed `tool-responses-auto.json`) and verified current source can write trusted runtime containment.

Result pipeline fix:

- Agent upload containment now removes auto-generated FAIL Bug rows for a case that was contained/rejudged as BLOCKED. This prevents C-02-style output where the case row is correctly contained, but the Bug sheet still advertises a false P2 product bug.
- Server-side parsed result containment now applies the same cleanup to parsed bug rows generated by `agent_result_writer` / fallback ingestion.
- Runtime containment smoke now explicitly covers unsuffixed `tool-responses-auto.json`, matching the E-08 failure artifact instead of only suffixed auto-response fixture names.

TAG_TOOL domain / fixture gaps identified from R0007:

- `CAPABILITY_GATE_degraded` and `helper pre-run skipped` appeared broadly. This means Codex was mostly driving live browser manually; it is a TAG domain-pack/helper maturity issue, not a direct product signal.
- Required domain pack/action-contract expansions include row menu opening, stable row selection by tag type/status/time type, condition-tag create/edit, manual upload chip lifecycle, manual upload validation, tag info table/chart reads, variable settings read/write flows, and safe destructive flow contracts.
...
### 2026-06-27 - TAG_TOOL R008 helper gate and result workbook repair

- 背景：NU_TAG_p0_R0008 / run `e1e365d4-e927-4866-948e-d29d861eb01c` 回 `PASS=11 / FAIL=2 / BLOCKED=59 / PARTIAL=1 / PENDING=15`。逐讀 report/archive 後，主因不是產品 regression：39 個 BLOCKED 是 `RESULT_TOOL_EXECUTION_UNAVAILABLE_WITHOUT_PREFLIGHT`，其上游是 TAG `manual_ai` cases 被 capability gate 降成 `CAPABILITY_GATE_degraded` 後跳過 helper；K-04 之後 15 個 PENDING 是 `RESULT_XLSX_SELF_CHECK_FAILED RESULT_XLSX_HEADER_MISSING`，因 Codex-generated `output/result.xlsx` 的 `Bug` sheet 存在但 header row 為空。
- Runtime gate：`case-feature-detection` 將 `playerTag.*` 視為 TAG_TOOL mode。`capability-gate` 與 `helper-execution-plan` 新增 `playerTag.* -> tagTool.*` 正規化：`createConditionalTag` / `createManualTag` 走 approval-gated create helper，`saveTagVariables` 依 `modifyValues` 判斷 read-only observe 或 approval-gated save，delete/terminate 只走 dangerous-modal cancel，不自動確認不可逆操作。
- Route inference：修正 TAG `manual_ai` cases 被 generic manual-AI degradation filter 清空 helper templates 的問題；該 filter 現在不套用到 TAG_TOOL。同步收斂 TAG inference 優先序，避免側欄/row/list 題因文字含「標籤變數設定」「條件標籤」「人工標籤」「刪除 icon」而誤導到 settings/create/upload/danger helper。新增負向語意 `不進行` / `不執行` / `不觸發`，避免「不進行刪除或終止」被判成危險操作。
- Result contract：`result-workbook-repair` 新增 empty/missing `Bug` sheet header repair。Codex-generated single-case result 若 `Bug` sheet header row 為空，會在 self-check 前補入 adapter headers；若 sheet 缺失則建立空 `Bug` sheet。若 case 是 FAIL 但仍無 linked Bug row，既有 `RESULT_FAIL_BUG_ROW_MISSING` 仍會擋下，不會把產品 bug 交付資訊吃掉。
- Smoke / 驗證：修前讀 R008 archive 確認 71 個 helper pre-run skipped、原因皆為 `CAPABILITY_GATE_SKIPPED_HELPER:CAPABILITY_GATE_degraded`，且 K-04 helper evidence 已成功但 workbook self-check 因 Bug header missing terminal。修後用 R008 實際 `testcase.md + case-manifest.json` 做離線 gate/plan smoke：`supported:true = 88/88`、skipped `0`，helper 分布為 list 34、create form 20、CSV upload 18、variable settings 11、danger modal cancel 3、Tool Bridge 2。已通過 `npm run verify:domain-pack -- --name TAG_TOOL`、`npm run verify:tag-tool-runtime`、`npm run verify:tag-tool-current-case-gate`、`npm run verify:agent-result-contract`、`npm run typecheck`、`npm run build --prefix agent`、`npm run build`、`git diff --check`。
- 邊界：本批是平台 runtime/gate/result-contract 修復，不改 BI/TAG 產品、不推 prod、不把 fixture gap 或 spec wording mismatch 改判產品 bug。R008 的 `E-02` example href mismatch 與 `E-05` toast wording mismatch 仍需以 live UI/PRD/testcase oracle 決定是 testcase/spec gap 或產品文案 gap；`A-05/B-04/H-04` 屬 fixture/data 前置 gap。

### 2026-06-27 - Dev accuracy profile: relax token-saving summary-first policy

- 背景：Tommy 指出目前已改用 `gpt-5.4-mini` 且成本較可控，過去為節省 token / 加速所加的讀取策略可能讓工具判讀不穩。既有 dev config 仍為 `codex_reasoning_effort=low`，task prompt 也有 `summary-first`、`straightforward PASS` 可不讀完整規則/文件的語句；這些在早期長 session、5.3 額度緊張時合理，但現在對 UAT 準確率不利。
- Runtime config：`defaultAgentConfig` 的 reasoning default 從 `low` 調為 `medium`。本機 dev config `/Users/tommy/.uat-agent-dev/config.json` 也同步改成 `codex_reasoning_effort=medium`；prod 未調整。
- Prompt policy：`task-runner` 的 generated `AGENTS.md`、run brief、Codex input summary、Codex prompt、auto-resume prompt 從 `summary-first` 改成 `evidence-first`。Summary 現在只作為導覽，不可取代 current-case pack、helper/capability/result evidence、domain/action/evidence contract 或 full rule text。
- 強制展開邊界：TAG_TOOL、CSV/download、fixture/testcase/spec gap、metadata/dropdown、Tool Bridge/destructive/native dialog、unsupported/degraded capability、helper contradiction、formula tool limitation、product bug/Bug row，以及所有 FAIL/BLOCKED/PARTIAL 都必須開 relevant full sources，不得以省 token 為理由只看 compact summary。
- 邊界：沒有回到「每題讀全部大型規則書」；仍使用 `rule-index` / `current-case-pack.mustReadRuleKeys` 定位精確文件，避免無意義暴增 token。這是平台執行政策與 dev runtime profile 調整，不改 domain pack、不改產品、不推 prod。

### 2026-06-27 - TAG_TOOL R0009 row/info/edit helper route repair

- 背景：NU_TAG_p0_R0009 / run `0cefb207-230c-4ce5-9205-bc8532892c48` 首次完整跑完 `88/88`，但仍有 `PASS=41 / FAIL=6 / BLOCKED=33 / PARTIAL=8`。逐讀 report/archive 後，主要 blocked 不應解讀為產品壞；高占比來源是 helper route/evidence 不足：C/F/M row action 只停在列表、G 編輯上傳誤導到新增頁、H/I/J 資訊頁只收列表 evidence、L modal 因 row 定位與 result gate 誤擋。
- Pre-fix smoke：用 R0009 實際 `input/cases` 做離線 route smoke，確認 `C-05 -> observeCreateForm`、`F-01/J-* -> observeList`、`G-* -> create page upload`、`L-04 -> TAG_ROW_NOT_FOUND_FOR_DANGEROUS_MODAL` 等錯誤可重現。這些都是平台 helper route/template gap 或 fixture/testcase gap，不是產品 regression。
- Runtime route：`capability-gate` 與 `helper-execution-plan` 新增 TAG 共用 helper template：`tagTool.openRowActionAndObserve`、`tagTool.observeTagInfo`、`tagTool.observeManualEditForm`。Route inference 會區分 row menu 多路由驗證、條件/人工資訊頁、人工編輯頁與 edit CSV upload；並忽略 cleanup /「本題不測項目」文字，避免 `篩選=不影響`、`manualUpload` evidence id、`不可回覆` PRD 文案污染 route。
- Executor：`bi-ui-helper-executor` 新增 TAG row finder、row menu action、tag info DOM reader、manual edit DOM reader。`uploadManualCsv` 支援 edit mode，會先從人工標籤 row 進入編輯頁再上傳 edit CSV。Dangerous modal helper 支援 toolbar selection delete（勾選 row 後點右上刪除並取消）與 row menu terminate；仍不確認刪除/終止，不要求 Tool Bridge。
- Domain fixture：新增 `invalidEditCsv` fixture `manual-tag-edit-invalid-operation.csv`，用於 G-05「操作欄非 add/update/delete」；`fixtures/requirements.json` 同步宣告。`editableManualTag` 資源本身仍是 missing domain fixture，若 dev 沒有安全可編輯人工標籤，仍應判 `BLOCKED_FIXTURE_MISSING_EDITABLE_MANUAL_TAG`，不改判產品 bug。
- Smoke / 驗證：已通過 `npm run verify:tag-tool-runtime`、R0009 實際 case 離線 route smoke、`npm run verify:tag-tool-current-case-gate`、`npm run verify:domain-pack -- --name TAG_TOOL`、`npm run typecheck --prefix agent`、`npm run typecheck`、`npm run build --prefix agent`、`npm run build`、`git diff --check`。
- 邊界：本批是 TAG_TOOL 平台 helper / route / domain fixture 修復，不改 Galaxy 產品、不推 prod。預期改善 C/F/G/H/I/J/L/M 的 `EVIDENCE_INSUFFICIENT`、`BLOCKED_RESULT_GATE_CONTAINMENT`、`BLOCKED_TOOL_LIMITATION` 類 blocked；A-05/B-04/H-04/L-02 等 fixture/data/testcase gap 仍需測試資料或 testcase oracle 補齊。
