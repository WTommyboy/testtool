...
- Codex Runner 指令修正：prompt 明確要求如果 MCP 只剩 navigation/tab/resize，但 `input/helper-execution-plan.json` 仍有可執行 helper action，不可直接判 `EVIDENCE_INSUFFICIENT`；應透過 shell command_execution 逐一跑 helper executor。遇到 `requiresToolBridge=true` 的 action 時，必須先輸出 Tool Bridge request，收到 Mac Agent auto response 後再帶 `--approved-tool-request-id` 執行該單一 action。
