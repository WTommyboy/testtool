...
- 驗證：已跑 `npm run build --prefix agent`、`npm run typecheck --prefix agent`、`npm run typecheck`、`npm run build`、`npm run build --prefix web`、supporting-docs profile smoke、A-03 artifact revised comparison smoke、`npm run verify:helper-hints`、`npm run verify:capability-gate`、`npm run verify:helper-report-gate`、`npm run verify:result-evidence-gate`、`npm run verify:agent-result-contract`、`npm run verify:final-aggregate-result`、`npm run verify:package-consistency`、`npm run verify:case-advance-policy`、`npm run verify:tool-bridge`、`npm run verify:agent-resume`、`npm run verify:agent-roundtrip`、`git diff --check` 通過。此變更是 runtime 行為變更，需推 deployment branch 並確認 Railway `/version`、`/health`。
