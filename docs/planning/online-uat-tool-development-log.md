...
- Railway：production `/version` 與 `/health` 已確認 healthy；截至 promote closeout 時 `/version` 顯示 branch `codex/uat-tool-mvp`、commit `3c79ebb`、deployment `f5d47f5c-22d4-4687-b153-72ae2addadb8`。`05d6934` 是 ancestry merge，tree 與 `3c79ebb` 相同，因此 production runtime 行為等同已部署的 `3c79ebb`。
