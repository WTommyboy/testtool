...
- 驗證：已跑 `npm run typecheck --prefix agent`、`npm run build --prefix agent`、`npm run typecheck`、`npm run build`、`npm run verify:capability-gate`、`npm run verify:source-prefilled-results`、`npm run verify:tool-bridge`、`npm run verify:result-evidence-gate`、`git diff --check` 通過。尚未 commit、push、deploy 或重啟 Mac Agent。
