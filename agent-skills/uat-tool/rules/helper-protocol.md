...
- Helper 若支援 `collage.saveReport`,儲存後應預設 best-effort 收集 `reportListEvidence`: 返回清單 readiness、saved report row 是否找到、row text、attempts 與 recovery actions。這是 helper contract,不需要 testcase 另外要求。若舊 helper report 缺少此 key, Codex 可引用 save API 200、reportName、已知成功/返回 dialogs 與 current-run DOM/list signals 判斷；不可只因 key 缺失就 BLOCK。
