import type { ParsedBug, ParsedResultCase, ParsedResultXlsx } from "./result-xlsx-parser";
...
const normalizeCaseKey = (value: string | null | undefined): string =>
  (value ?? "")
    .trim()
    .replace(/\s+/g, "")
    .replace(/^DEMO-/i, "")
...
const isAutoGeneratedFailBug = (bug: ParsedBug): boolean => {
  const text = [
    bug.bugId,
    bug.title,
    bug.description,
  ...
  return /auto_generated_from_fail\s*=\s*true/i.test(text) ||
    /source\s*=\s*(?:agent_result_writer|result_xlsx_ingest_fallback)/i.test(text);
};

const removeContainedAutoGeneratedFailBugs = (parsed: ParsedResultXlsx, updatedCaseNos: string[]): void => {
  if (updatedCaseNos.length === 0 || parsed.bugs.length === 0) return;
  const updatedCaseKeys = new Set(updatedCaseNos.map(normalizeCaseKey).filter(Boolean));
  parsed.bugs = parsed.bugs.filter((bug) => {
    const relatedKey = normalizeCaseKey(bug.relatedCaseNo);
    return !(updatedCaseKeys.has(relatedKey) && isAutoGeneratedFailBug(bug));
  });
};
...
export const containCaseLevelResultEvidenceGateIssues = (
  ...
  removeContainedAutoGeneratedFailBugs(parsed, updatedCaseNos);
